...
func main() {
	...
	createdUser, resp, err := apiClient.UsersApi.CreateUser(ctx, swagger.CreateUserSchema{
		Name:     "A name",
		Email:    "an email",
		Username: "myusername",
		//RootRole:  *swagger.OneOfcreateUserSchemaRootRole(1),
		SendEmail: false,
	})
	fmt.Println(createdUser, resp, err)
}
